    ...
    protected static throwsIfNotListener() {
        assert.doesThrow(() =>
            eventDiskUtil.splitPathToListener(
                this.resolvePath(
                    'src/listeners/my-great-event/test-event.listener.ts'
                )
            )
        )
        ...
        assert.doesThrow(() =>
            eventDiskUtil.splitPathToListener(
                this.resolvePath(
                    'my-great-event/v2020_02_02/my-event.listener.ts'
                )
            )
        )
    ...
    protected static throwsIfNotEvent() {
        assert.doesThrow(() =>
            eventDiskUtil.splitPathToEvent(
                this.resolvePath(
                    'src/events/my-great-event/emitPayload.builder.ts'
                )
            )
        )
    ...
    protected static canResolveListenerPathWithNumbersInNamespace() {
        ...
        assert.isEqualDeep(eventDiskUtil.splitPathToListener(eventPath), {
            ...
            fullyQualifiedEventName:
                'some-great-skill-1234.did-book::v2020_10_10',
            version: 'v2020_10_10',
        })
    }
